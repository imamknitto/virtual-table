...
const HeaderFilter = ({ isShowFilter, headerKey }: IHeaderFilter) => {
  const filterSearch = useTableContext((ctx) => ctx.filterSearch);
  const filterSelection = useTableContext((ctx) => ctx.filterSelection);
  const filterAdvance = useTableContext((ctx) => ctx.filterAdvance);
