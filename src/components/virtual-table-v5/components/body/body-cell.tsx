...
function BodyCell<TData>(bodyCellProps: IBodyCell<TData>) {
  ...
  const classNames = useMemo(() => {
    if (freezeMode === 'left') {
      return clsx('table-cell border-r bg-white truncate', {
    ...
    if (freezeMode === 'right') {
      return clsx('table-cell border-l bg-white truncate', {
        '!bg-[#F6F6FF]': isRowHighlighted,
    ...
    return clsx('table-cell border-r nth-last-[1]:!border-r-transparent truncate', {
      'bg-[#F6F6FF]': isRowHighlighted,
