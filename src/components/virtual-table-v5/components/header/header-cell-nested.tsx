...
import { DEFAULT_SIZE, findChildRecursive, type IAdjustedHeader } from '../../lib';
...
function HeaderCellNested(props: INestedHeaderCell) {
  ...
  const getScaledWidth = (parentKey: string, parentVirtualIndex: number, childWidth: number = 0) => {
    ...
    const parent =
      freezeType === 'left'
        ? freezeLeftColumns.find((col) => col.key === parentKey)
        : freezeType === 'right'
        ? freezeRightColumns.find((col) => col.key === parentKey)
        : columns.find((col) => col.key === parentKey);
