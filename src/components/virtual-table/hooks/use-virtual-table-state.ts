...
export function useVirtualTableState<TData>(virtualTableProps: IVirtualTable<TData>) {
  ...
  const {
    ...
    regularHeaders,
    sumHeaderWidth,
    sumHeaderStickyLeftWidth,
    sumHeaderStickyRightWidth,
  } = useMemo(() => {
    let stickyLeftWidth = 0;
    let stickyRightWidth = 0;
    let totalWidth = 0;

    const sticky: typeof memoizedVisibleHeaders = [];
    const regular: typeof memoizedVisibleHeaders = [];

    for (const header of memoizedVisibleHeaders) {
      const width = header.width || DEFAULT_SIZE.COLUMN_WIDTH;
      totalWidth += width;

      if (header.sticky === 'left') {
        stickyLeftWidth += width;
        sticky.push(header);
      } else if (header.sticky === 'right') {
        stickyRightWidth += width;
        sticky.push(header);
      } else {
        regular.push(header);
      }
    ...
    return {
      ...
      regularHeaders: regular,
      sumHeaderWidth: totalWidth,
      sumHeaderStickyLeftWidth: stickyLeftWidth,
      sumHeaderStickyRightWidth: stickyRightWidth,
    };
  }, [memoizedVisibleHeaders]);
  ...
  const columnVirtualizer = useVirtualizer({
    horizontal: true,
    count: regularHeaders.length,
    getScrollElement: () => scrollElementRef.current,
    estimateSize: (index) =>
      (regularHeaders[index].width || DEFAULT_SIZE.COLUMN_WIDTH) +
      (regularHeaders[index].noStretch ? 0 : extraWidth),
    overscan: 10,
  });
  ...
  const handleResizeColumn = useCallback(
    (headerKey: string, columnIndex: number, newWidth: number) => {
      // Debounce resize updates untuk mengurangi re-render yang berlebihan
      const timeoutId = setTimeout(() => {
        ...
        // Hanya resize virtualizer jika benar-benar diperlukan
        if (columnVirtualizer) {
          columnVirtualizer.resizeItem(columnIndex, newWidth);
        }
      }, 50); // Debounce 50ms untuk mengurangi update yang berlebihan
  ...
  const tableHasSticky = tableHeaders.some((col) => col.sticky);
  ...
  const tableBodyTopPosition = tableHasSticky
    ? 0
    : headerHeight ||
