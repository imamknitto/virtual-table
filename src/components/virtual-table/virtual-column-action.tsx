...
const VirtualColumnAction = (props: IVirtualColumnAction) => {
  ...
  const contextValues = useTableContext((ctx) => ({
    ...
    columnVisibilityList: ctx.columnVisibilityList,
    handleToggleColumnVisibility: ctx.handleToggleColumnVisibility,
  }));

  const { isFilterVisible, columnVisibilityList, handleToggleColumnVisibility } = contextValues;
  ...
  const handleClickOutside = useCallback(() => {
    if (showActionCard.show) setShowActionCard((prev) => ({ ...prev, show: false }));
    setShowColumnVisibility((prev) => ({ ...prev, show: false }));
  }, [showActionCard.show]);

  useClickOutside([wraperRef, columnVisibilityRef], handleClickOutside);

  // Handler untuk toggle action card
  const handleClickActionToggle = useCallback((e: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    e.stopPropagation();
  ...
  const actionMap = useMemo(() => ({
    ...
    'Hide/Filter Kolom': (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
      const rect = e.currentTarget?.getBoundingClientRect();
  ...
  const handleClickAction = useCallback(
    (action: string, e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
      e.stopPropagation();
      actionMap[action as keyof typeof actionMap]?.(e);
    },
    [actionMap],
  );
