...
const VirtualTableBody = forwardRef(<TData,>(props: IVirtualTableBody<TData>, ref: React.Ref<HTMLDivElement>) => {
  ...
  const { freezeLeftColumns, freezeRightColumns, freezeRightColumnsWidth, freezeLeftColumnsWidth, columns, getLeaves } =
    useHeaderContext();
  ...
  const { useFooter, expandedContent, calcTotalTableWidth, freezeColLeftPositions, freezeColRightPositions } =
    useUIContext();
    ...
    return freezeRightColumns.flatMap((column, freezeRightIdx) => {
      ...
      return [
        <BodyCell
          ...
          position={{ left: freezeColRightPositions[freezeRightIdx], width: column.width!, height: rowHeight }}
        />,
      ];
    });
  };
